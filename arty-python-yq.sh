...
# Version: 1.0.1 - Python yq compatibility
...
# Detect which version of yq is installed
detect_yq_version() {
    local yq_help=$(yq --help 2>&1 || true)
    local yq_version=$(yq --version 2>&1 || true)
    
    if echo "$yq_version" | grep -qi "mikefarah"; then
        echo "go"
    elif echo "$yq_help" | grep -q "jq_filter" || echo "$yq_version" | grep -qi "python"; then
        echo "python"
    else
        echo "unknown"
    fi
...
check_yq() {
    ...
    if [[ "$YQ_TYPE" == "unknown" ]]; then
        log_warn "Unknown yq version detected. Assuming Go version (mikefarah/yq)"
        YQ_TYPE="go"
    fi
}
...
get_yaml_script() {
    ...
    if [[ "$YQ_TYPE" == "python" ]]; then
        yq -r ".scripts.${script_name} // \"null\"" "$file" 2>/dev/null || echo "null"
    else
...
list_yaml_scripts() {
    ...
    if [[ "$YQ_TYPE" == "python" ]]; then
        yq -r '.scripts | keys[]' "$file" 2>/dev/null
    else
...
exec_script() {
    ...
    if [[ -z "$cmd" ]] || [[ "$cmd" == "null" ]]; then
        ...
        local scripts_output=$(list_yaml_scripts "$config_file")
        if [[ -n "$scripts_output" ]]; then
            ...
            echo "  (no scripts defined in arty.yml)"
        fi
        return 1
    fi
